...
classdef DS_V2 < StimGLProtocol
    
    % Protocol present moving circles in as much as 8 directions, and show
    % spike rates as a function of direction, speed and diameter of the
    % circle (in microns or time residency).
    % 0degres = horizontal right of center.

    properties (Constant)
        identifier = 'org.janelia.research.murphy.stimgl.movingobjects'
        version = 1
        displayName = 'Direction Selectivity C'
    ...
    properties
        ...
        objectDiameter = 50:50:150;  % microns 
        unitObjDiam= { 'microns', 'constant time residency (sec/um)' }
...
%         WholeCell = false;
    ...
    methods
        ...
        function prepareRun(obj)
            ...
            obj.trialTypes = allcombs(obj.objectDiameter,obj.objectSpeed,obj.objectDir);
            ...
            if numel(obj.objectDiameter)>1
                obj.plotData.meanDiamResp = NaN(1,numel(obj.objectDiameter));
                obj.openFigure('Custom','Name','MeanDiamRespFig','UpdateCallback',@updateMeanDiamRespFig);
            end
        end
        ...
        function updateMeanDiamRespFig(obj,axesHandle)
            if obj.epochNum==1
                obj.plotData.meanDiamRespHandle = line(obj.objectDiameter,obj.plotData.meanDiamResp,'Parent',axesHandle,'Color','k','Marker','o','LineStyle','none','MarkerFaceColor','k');
                set(axesHandle,'Box','off','TickDir','out','XLim',[min(obj.objectDiameter)-10,max(obj.objectDiameter)+10],'Xtick',obj.objectDiameter);
                if strcmp(obj.unitObjDiam,'microns')
                    xlabel(axesHandle, 'Circle Diameter (um)');
                else
                    xlabel(axesHandle, 'time spend for each position (sec)');
                end
                ylabel(axesHandle,'response (spike count)');
            else
                set(obj.plotData.meanDiamRespHandle,'Ydata',obj.plotData.meanDiamResp);
            end
            line(obj.plotData.epochObjectDiam,obj.plotData.epochResp,'Parent',axesHandle,'Color','k','Marker','o','LineStyle','none');
        end
        ...
        function prepareEpoch(obj)
            ...
            epochObjectDiam = obj.trialTypes(epochTrialType,1);
            ...
            if strcmp(obj.unitObjDiam, 'microns')
                XsizeVectorPix = [epochObjectDiam / obj.Yfactor * ones(1,appFrames + nStimFrames),zeros(1,(obj.dispTime+5)*frameRate)];
            else
                XsizeVectorPix = [epochObjectSpeed * epochObjectDiam / obj.Yfactor * ones(1,appFrames + nStimFrames),zeros(1,(obj.dispTime+5)*frameRate)];
            end
            ...
            obj.addParameter('epochObjectDiam',epochObjectDiam);
            ...
            stimulus = zeros(1,floor(obj.rigConfig.sampleRate*(obj.preTime+appFrames+stimTime+obj.dispTime+obj.postTime)));
        ...
        function completeEpoch(obj)
            ...
            if numel(obj.objectDiameter)>1
                objectDiamIndex = find(obj.objectDiameter==obj.plotData.epochObjectDiam,1);
                if isnan(obj.plotData.meanSizeResp(objectDiamIndex))
                    obj.plotData.meanDiamResp(objectDiamIndex) = obj.plotData.epochResp;
                else
                    obj.plotData.meanDiamResp(objectDiamIndex) = mean([repmat(obj.plotData.meanDiamResp(objectDiamIndex),1,obj.loopCount-1),obj.plotData.epochResp]);
                end
            end
